  ...
  return (
    <nav className={`px-custom w-full flex items-center justify-between py-5 fixed top-0 z-20`}>
      ...
      <div className="hidden md:flex">
        <button className="text-white h-10 px-5 pink-gradient-background rounded-full mr-3 scale-10" onClick={goBack}>Go Back</button>
        <button className="text-white" onClick={goBack}>Home</button>
      </div>
      <div className="md:hidden flex flex-1 justify-end items-center">
        ...
        <div className={`${ !toggle ? "hidden" : "flex" } p-6 landing-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
          ...
          <li>
            <a href="#" onClick={goBack} className="cursor-pointer">Go Back</a>
          </li>
          </ul>
        </div>
      </div>
